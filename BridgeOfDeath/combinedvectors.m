...
for n = 1:length(u)% loop through each of the points
    ...
    plot(r(1,:),r(2,:)), axis([-2 2 -2 2]),hold on
    plot(Ydis,-Xdis,'--');
...
for n = 1:30:length(u)
    quiver(r(1,n),r(2,n),That(1,n),That(2,n),'r'); % plot the unit tangent
    quiver(r(1,n),r(2,n),Nhat(1,n),Nhat(2,n),'b');
end
hold off
title('combined path')
xlabel('x position')
...
legend({'unit normal vector','unit tangent vector','planned path','recorded path'},'Location','southwest')